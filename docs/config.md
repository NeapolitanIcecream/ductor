...
| `file_access` | `str` | `"workspace"` | File access scope (`workspace`, `home`, `all`) for file sends and API `GET /files`; unknown values fall back to workspace-only |
...
| `allow_public` | `bool` | `false` | Allows a non-loopback, non-Tailscale bind |
